...
Over the summer, my goal is to port Rover to the ROS platform to be able to run
tests on real quadrotors not in simulation. Once it has been ported, I would
like to experiment with different types of sensed risk instead of simply
modelling it \emph{a priori}. One way that this will be possible would be to
put a temperature sensor on the robot and use heat as the risk function. This
is applicable in a fire evacuation when having risk aware coverage of an area
minimizes damage to the quadrotors and maximizes the amount of information
gained about the disaster. I would also like to extend Rover to use different
potential surfaces than simply the time grid. In particular, I would like to
use a multimodal probability density function that models the probability of a
target being at a certain location at a given point in time. This probability
density function will assume that the object being tracked is not moving
randomly but rather, is cohering to a certain parametric model of movement. By
observing the object's previous states, we are able to use several techniques
to predict its model of motion. Once the target's model of motion is known, we
can generate a more informed probability density function about its future
movements. This probability density function can be used to replace the current
time grid being used. The time line for the internship is shown in
Table~\ref{tab:timeline}

\begin{table}
    \begin{tabular}{p{30mm}p{100mm}}
    Dates        & Description \\ \hline
    Jun 16 - 30  & Finish paper for Rover for simple persistent risk and sensor quality aware coverage. Submitting to ROBIO 2014 \\ \hline
    Jul 1 - 29   & Develop algorithms for informative target tracking using a predictive potential surface. Add the developed algorithms to Rover. Work on the paper for submission to ICAART 2015. Also, get the project to work on the quadrotors for the capstone project. \\ \hline
    Jul 30 - End & Write any needed documentation. \\ 
    \end{tabular}

    \caption{Time line for the NREIP internship}
    \label{tab:timeline}
\end{table}
