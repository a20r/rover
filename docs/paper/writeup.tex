...
determined using the same functions as in the optimization process. For the
...
each amount of quadrotors was run on every configuration of risk points.  The
number of risk points varied from 1 to 21 with a step of 2 and the number of
quads varied from 1 to 26 with a step of 5.  Each configuration was run for 250
iterations and each configuration only needed to be tested once because the
process is deterministic. 

\subsection{Percentage of Total Area Coverage}
The percent of the total area coverage is used to show that the quadrotors fill
the workspace as described in the problem description. It was determined using
a Monte Carlo process where a large number of random points (1000) with $x$ and
$y$ values within the search area were checked to be within any of the sensing
ellipses at each iteration.  The ratio of the number of points sampled which
are within any of the sensing ellipses to the total number of points sampled is
what determines the percent of total area coverage metric.

\subsection{Average Time Difference}
The average time difference within the time grid is used to show that no area
in the grid goes for too long a time without being sensed. As the number of
quadrotors increases, we expect that the average time difference to decrease
because the quadrotors are spreading through the space cooperatively and
therefore cover the space more quickly.  This metric can be interpreted as the
average number of iterations that have gone by since viewing any cell in the
time grid. This metric was determined by iterating through the grid and
subtracting the current time from the time retrieved from the grid. 
