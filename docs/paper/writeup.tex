...
This is shown in
\textbf{Algorithm} \ref{algo:2dplanning}. This simple rule has extreme emergent
properties for the swarm. Since the swarm shares the same grid, the quads will
act cooperatively to fill the space without having to provide much guidance.
This is shown in
...
$$ E(\theta) = \mathlarger{\frac{\mathlarger{\sum_{s \in
S(\theta)}{\mathcal{T}[s.x, s.y]}}}{|S(\theta)|}} $$
...
An instance of the time grid with a non-zero camera angle is shown in
\fig{timegrid}.
