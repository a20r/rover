...
the pseudocode, $\phi$ is the camera angle, $\alpha$ is the viewing angle of
the camera, and $A_{M}$ and $A_{m}$ are the respective semi-major and
semi-minor axes.
...
\STATE $A_{M} \leftarrow q.\Var{z} \cdot (\tan{(\phi + \alpha)} - \tan{\phi}) + \epsilon$
\STATE $A_{m} \leftarrow q.\Var{z} \cdot \frac{\tan{\alpha}}{\cos{\phi}} + \epsilon$

\STATE $x \leftarrow q.\Var{z} \cdot \tan{(\phi - \alpha)} + A_{M} \cdot (1 + \cos{\theta})$
\STATE $y \leftarrow A_{m} \cdot \sin{\theta}$
...
\IF{$\mathcal{T}[x_r, y_r] < t_{min}$}

\STATE $t_{min} \leftarrow \mathcal{T}[x', y']$
...
\STATE $(x', y') \leftarrow \Function{Translate}(\begin{bmatrix}
    q.\Var{x} &
    q.\Var{y}
\end{bmatrix}^T, \hat{v})$
