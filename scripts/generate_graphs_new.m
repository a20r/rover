...
function [avg_ta, avg_sq, avg_risk, avg_time] = getAverages(rn, qn)
    data = load(strcat("data/out_", num2str(rn), "_", num2str(qn), ".txt"));
    avg_ta = mean(data(:, 1));
    avg_sq = mean(data(:, 2));
    avg_risk = mean(data(:, 3));
    avg_time = mean(data(:, 4))
...
function [t_avg_ta, t_avg_sq, t_avg_risk, t_avg_time] = getAverageMatrices(rns, qns)

    t_avg_ta = zeros(size(rns)(2), size(qns)(2));
    t_avg_sq = zeros(size(rns)(2), size(qns)(2));
    t_avg_risk = zeros(size(rns)(2), size(qns)(2));
    t_avg_time = zeros(size(rns)(2), size(qns)(2));

    for ri = 1:size(rns)(2)
        for qi = 1:size(qns)(2)
            [avg_ta, avg_sq, avg_risk, avg_time] = getAverages(rns(ri), qns(qi));
            t_avg_ta(ri, qi) = avg_ta;
            t_avg_sq(ri, qi) = avg_sq;
            t_avg_risk(ri, qi) = avg_risk;
            t_avg_time(ri, qi) = avg_time;
        end
    end
endfunction
...
function plotAll(rns, qns)
    ...
    [t_avg_ta, t_avg_sq, t_avg_risk, t_avg_time] = getAverageMatrices(rns, qns);
    figure;
    % hold on;
    subplot(2, 2, 1);
    ...
    subplot(2, 2, 4);
    surf(QNS, RNS, t_avg_time);
    xlabel("Number of Quads");
    ylabel("Number of Risk Points");
    title("Average Time Difference");
endfunction
